...
url=paste0("http://hgdownload.cse.ucsc.edu/goldenPath/",build,"/database/gwasCatalog.txt.gz")

# only download if the new version has been updated (wget -N)
if("gwasCatalog.txt.gz"%in%list.files()) {

    system(paste0("mv gwasCatalog.txt.gz gwasCatalog.prev.txt.gz "))
    system(paste0("wget -N ",url))
