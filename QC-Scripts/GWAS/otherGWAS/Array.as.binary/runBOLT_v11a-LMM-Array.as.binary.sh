...
vers=v11a  # this version is the same as v11, but uses the sex chromosomes from clare's set. That is, without excluding Array test snps.
...
genoDataAll=$basedir/data/Combined/byChrom/b1__b11-b001__b095-sexchrom-oxfordqc

chr=$SGE_TASK_ID

thisChromGenoData=$genoDataAll-chr$chr

# chr: 23 = X; 24 = Y; 25 = XPAR; 26 = MT.
# sex chromosomes are subset in subset-sex-chroms.sh

phenoFile=$basedir/data/GWAS/otherGWAS/PhenotypesForBOLT-v11-$chr.txt  # using White british subset. Note: some individuals will be excluded in the genotype data
...
--bim=$thisChromGenoData-recoded.bim \
--phenoFile=$phenoFile \
--phenoCol=$pheno \
--covarFile=$phenoFile \
...
# qsub -t 23-26 -q short.qc -P donnelly.prjc runBOLT_v11a-LMM-Array.as.binary.sh
