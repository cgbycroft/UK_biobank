...
#args=c("/well/ukbiobank/qcoutput.V2_QCed.sample-QC/data/GWAS/otherGWAS/Internal.Pico..ng.uL..qnorm/BOLTLMM.v16/Internal.Pico..ng.uL..qnorm-BOLT-LMM-v16.out","all","plots","-ymax","50","-phenoFile","/well/ukbiobank/qcoutput.V2_QCed.sample-QC/data/GWAS/otherGWAS/PhenotypesForBOLT-v3.txt","-title","-raw","-dontPlotRegions")

#args=c("/well/ukbiobank/qcoutput.V2_QCed.sample-QC/data/GWAS/otherGWAS/Standing.height/BOLTLMM.v16s/Standing.height-BOLT-LMM-v16s.out","23","plots","-ymax","50","-phenoFile","/well/ukbiobank/qcoutput.V2_QCed.sample-QC/data/GWAS/otherGWAS/PhenotypesForBOLT-v3.txt","-title","-raw","-dontPlotRegions")
...
results = read.gwas(dataFile,chrom="genome",minmaf=minmaf,mininfo=mininfo,maxmiss = maxmiss,Ymax=Ymax,QCexclusions=QCexclude,extraTitle=extraTitle,useLmmInf=useLmmInf)
...
if( !( "HITSchrom"%in%ls() )&(!"LDchrom"%in%ls() ) ){
    ...
    LDchrom = vector("list", 22)

    for (chrom in chroms){

        print(chrom)
        genotypeFile2=genotypeFile

        if(chrom==25){
            genotypeFile2 = paste0(genotypeFile,".chrXY")
        }
        if(chrom==23) genotypeFile2 = paste0(genotypeFile,".chrX")
...
#### compute LD with top SNPs (if they exist, and were asked for)
        if( length(HITS$topVariants) > 0 ){
            ...
            if(computeLD){                                
                ld = compute.ld.around.snp(HITS$topVariants,genotypeFile2,chrom,paste0(samplesFile,".samples.tmp"),bgenSampleFile,DF)
...
################################### 
# plot hit regions over whole chromosomes (always do this)
...
if(saveData){                
    ...
    print("Saving LD around hits data to:")
    print(paste0(results$Pvalset,"-hitsAndLDcalc.RData"))
}
                                        # This will actually also be saved later along with rare stuff, but just in case the bits below fail, we at least have a data set with the ld info!
...
#### plot the regions
        if(length(HITS$topVariants) > 0){
            sapply(1:length(HITS$topVariants),FUN=function(s) {
                ...
                plot.gwas.region.v2(DF,HITS$topVariants[s],catFile,ld,plotDir=paste0(plotOutDir,"/Region.plots"),recombRates=recomb,region=regionToPlot,Ymax=50,width=10^6,moreToPlot=FALSE,Pvalset=Pvalset,addGenes=list("genes"=genes,"exons"=exons),showRegion=regionToPlot)
...
########################
# Look at rare variants

print("Any rare variants?")

# keep everything
#resultsRaw = read.gwas(dataFile,chrom="genome",minmaf=0,mininfo=0,maxmiss = 1,Ymax=50,QCexclusions=QCexclude,extraTitle=extraTitle)

# keep rare, but exclude based on qc if asked
results2 = read.gwas(dataFile,chrom="genome",minmaf=0,mininfo=mininfo,maxmiss = maxmiss,Ymax=Ymax,QCexclusions=QCexclude,extraTitle=extraTitle)
DF2 = results2$DF

rare = 0.001 # we filtered these out before
rareHits = (DF2$MAF <= rare) & (DF2$P< 5e-8)
print( sum(rareHits) )

rareHitSnps = c()
LDrare = list()
if( sum(rareHits) > 0 ){
    rareHitSnps = DF2$SNP[rareHits]  # there are 7 rare hit SNPs!

    for(chrom in unique(DF2$CHR[rareHits])){
        if(computeLD){            
            ld = compute.ld.around.snp(rareHitSnps,genotypeFile,chrom,paste0(samplesFile,".samples.tmp"),bgenSampleFile,DF)
            LDrare[[as.character(chrom)]] = ld
        } else {
            LDrare[[as.character(chrom)]]=NULL
        }

        if(plotRegions){

            sapply(1:length(rareHitSnps),FUN=function(s) {
                regionToPlot = DF2$BP[DF2$SNP==rareHitSnps[s]] + c(-(10^6)/2,(10^6)/2)
                chrom =  DF2$CHR[DF2$SNP==rareHitSnps[s]]
                ld = LDrare[[as.character(chrom)]]
                recomb = recombrates[[as.character(chrom)]]
                Pvalset = gsub("genome",chrom,results2$Pvalset)
                
                #plot.gwas.region(DF2,rareHitSnps[s],catFile,ld,plotDir=paste0(plotOutDir,"/Region.plots"),recombRates=recomb,region=regionToPlot,Ymax=50,width=10^6,moreToPlot=TRUE,Pvalset=Pvalset)
                plot.gwas.region.v2(DF2,rareHitSnps[s],catFile,ld,plotDir=paste0(plotOutDir,"/Region.plots"),recombRates=recomb,region=regionToPlot,Ymax=50,width=10^6,moreToPlot=FALSE,Pvalset=Pvalset,addGenes=list("genes"=genes,"exons"=exons),showRegion=regionToPlot)
...
################################### REQUIRED FOR CONDITIONAL ANALYSIS OR FURTHER PLOTTING
# output save
                
if(saveData){                
    save(HITSchrom,LDchrom,LDrare,rareHitSnps,file=paste0(results$Pvalset,"-hitsAndLDcalc.RData"))
}

                                        # print set of SNPs for conditional analysis
allHitSNPs = unlist(sapply(HITSchrom,function(x) x$topVariants))
write.table(allHitSNPs,file=paste0(results$Pvalset,"-topVariants.txt"),quote=FALSE,col.names=FALSE,row.names=FALSE)

write.table(rareHitSnps,file=paste0(results2$Pvalset,"-topVariants.txt"),quote=FALSE,col.names=FALSE,row.names=FALSE)
