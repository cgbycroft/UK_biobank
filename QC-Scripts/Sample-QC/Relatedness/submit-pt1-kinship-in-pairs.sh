...
king19=/well/ukbiobank/qcoutput.V2_QCed.sample-QC-testing/src/Linux-king-1.9/king

#NOTE: king19 is apparently faster, but not great gains are made with batch-pair approach. Storing the output is the main bottleneck.
...
if [[ -f $Batch1.bed && -f $Batch1.bim && -f $Batch1.fam && -f $Batch2.bed && -f $Batch2.bim && -f $Batch2.fam ]] ; then
    ...
    echo $plink --bfile $Batch1 --bmerge $Batch2.bed $Batch2.bim $Batch2.fam --allow-no-sex --make-bed --keep-allele-order --out $Merged
    time $plink --bfile $Batch1 --bmerge $Batch2.bed $Batch2.bim $Batch2.fam --allow-no-sex --make-bed --keep-allele-order --out $Merged
    ...
    echo $king14 -b $Merged.bed --prefix $Merged/degree3-init --kinship --related --degree 3
    time $king14 -b $Merged.bed --prefix $Merged/degree3-init --kinship --related --degree 3
