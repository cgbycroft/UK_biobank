...
do \
    ...
    # Get snp and sample order for interim release data (UKBileve and UKBiobank batches separately)
    paste -d' ' <( cut -f1 -d' ' $dataFileInterim | tr '[:lower:]' '[:upper:]' | tail -n +2 ) \
	  <( cut -f1 -d' ' $dataFileInterim | tr '[:lower:]' '[:upper:]' | tail -n +2 ) \
	  > $outDir/$batch.$i.interim.best.array.txt

    head -n 1 $dataFileInterim | tr ' ' '\n' | tail -n +3  > $outDir/$batch.$i.interim.snp.order.txt


    # Find all the individuals and snps in the new data release
    newData=$newDataDir/V2_QCed.export.UKBiLEVEAX_b1-b11.Batch_b001_b095.chr$i

    $plink --bfile $newData --keep $outDir/$batch.$i.interim.best.array.txt --keep-allele-order --recode A-transpose --out $basedir/data/Combined/interimReleaseSamples/V2_QCed.export.UKBiLEVEAX_b1-b11.Batch_b001_b095.chr$i.interim_samples_$batch


    head -n 1 $basedir/data/Combined/interimReleaseSamples/V2_QCed.export.UKBiLEVEAX_b1-b11.Batch_b001_b095.chr$i.interim_samples_$batch.traw | tr '\t' '\n'| tail -n +7 > $outDir/$batch.$i.new.best.array.txt

    awk '{print $2}' $basedir/data/Combined/interimReleaseSamples/V2_QCed.export.UKBiLEVEAX_b1-b11.Batch_b001_b095.chr$i.interim_samples_$batch.traw | tail -n +2 > $basedir/data/Combined/interimReleaseSamples/Comparison/$batch.$i.new.snp.order.txt
    ...
    Rscript countDifferences.R \
	    ...
	    $dataFileInterim \
	    $outDir/$batch.$i.interim.best.array.txt \
	    $outDir/${array}_$batch.chr$i.calls.ctsu_basic.compareToNew
